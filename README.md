...
A Claude Code skill for downloading and summarizing posts from Xiaohongshu (小红书/RedNote) — both video posts and image posts — using [yt-dlp](https://github.com/yt-dlp/yt-dlp).

## Features

- **Auto post-type detection** — same command works for both video posts and image posts
- Download Xiaohongshu videos in best available quality (up to 1080p)
- **Image post support** — downloads all images + post text, then generates an AI summary
...
- **AI-powered summary** — structured summary generated by Claude (always on by default)
- **Batch download** — queue multiple URLs in `download_list.md`, auto-marks completed items
...
```bash
python3 scripts/download_xiaohongshu.py "https://www.xiaohongshu.com/explore/VIDEO_ID" -o content/
```
...
The script auto-detects whether the URL is a video or image post. No flags needed:
...
└── summary.md         # AI-generated summary (Claude reads all images)
```

### Batch Download

Queue multiple URLs in `download_list.md` — one URL per line. Lines starting with `#` are skipped. After a successful download, the script automatically prefixes the line with `# [done] `.
...
Claude automatically generates `summary.md` after every download — no extra flag needed.
...
| `--full` | Full resource pack mode | `true` |
| `--summary` | Save `.meta.json` for Claude to generate `summary.md` | `true` |
| `--batch` | Batch download from a file of URLs | — |

### Examples

```bash
# Download a video post (auto-detected, full resource pack + AI summary by default)
python3 scripts/download_xiaohongshu.py "https://www.xiaohongshu.com/explore/69821980000000000e03c95f" -o content/

# Download an image post (auto-detected)
python3 scripts/download_xiaohongshu.py "https://www.xiaohongshu.com/explore/IMAGE_POST_ID" -o content/

# Batch download from download_list.md
python3 scripts/download_xiaohongshu.py --batch download_list.md -o content/

# Download in 720p
python3 scripts/download_xiaohongshu.py "URL" -q 720p -o content/

# Download audio only
python3 scripts/download_xiaohongshu.py "URL" -a -o content/

# Use Firefox cookies instead of Chrome
python3 scripts/download_xiaohongshu.py "URL" --browser firefox -o content/
...
8. Prepares `.meta.json` metadata for Claude to generate `summary.md`

For image posts, the tool:

5. Parses the post page using a mobile User-Agent to extract all image URLs
6. Downloads each image and saves post text to `post.txt`
7. Saves `.meta.json` for Claude to generate `summary.md` by reading every image
...
├── README.md                             # This file
├── LICENSE                               # MIT License
├── download_list.md                      # Batch download queue
├── .gitignore
├── scripts/
│   ├── download_xiaohongshu.py           # Main downloader script (video + image)
│   └── parallel_transcribe.py            # Parallel Whisper transcription
└── reference/
    ├── summary-prompt.md                 # AI summary prompt for video posts
    └── image-summary-prompt.md           # AI summary prompt for image posts
```
...
一个基于 [yt-dlp](https://github.com/yt-dlp/yt-dlp) 的 Claude Code 技能，用于下载和总结小红书 (RedNote) 的视频帖子和图文帖子。

## 功能特点

- **自动识别帖子类型** — 同一条命令，自动区分视频帖和图文帖
- 下载小红书视频，最高画质可达 1080p
- **图文帖支持** — 下载所有图片 + 正文，并生成 AI 摘要
...
- **AI 智能总结** — 由 Claude 生成结构化摘要（默认开启）
- **批量下载** — 在 `download_list.md` 中排队多个链接，自动标记已完成
...
```bash
python3 scripts/download_xiaohongshu.py "https://www.xiaohongshu.com/explore/视频ID" -o content/
```
...
脚本自动识别视频帖或图文帖，无需额外参数：

```bash
python3 scripts/download_xiaohongshu.py "链接" -o content/
```
...
├── post.txt           # 帖子标题和正文
├── .meta.json         # 帖子元数据
└── summary.md         # AI 摘要（Claude 读取所有图片后生成）
```

### 批量下载

在 `download_list.md` 中按行填入链接，`#` 开头的行会被跳过。下载成功后，脚本自动在该行前添加 `# [done] `。
...
每次下载完成后 Claude 会自动生成 `summary.md`，无需额外指令。

### 参数说明

| 参数 | 说明 | 默认值 |
|------|------|--------|
| `-o, --output` | 输出目录 | `content/` |
...
| `--full` | 完整资源包模式 | `true` |
| `--summary` | 保存 `.meta.json` 供 Claude 生成 `summary.md` | `true` |
| `--batch` | 从文件批量下载 | — |

### 使用示例

```bash
# 下载视频帖（自动识别，默认完整资源包 + AI 总结）
python3 scripts/download_xiaohongshu.py "https://www.xiaohongshu.com/explore/69821980000000000e03c95f" -o content/

# 下载图文帖（自动识别）
python3 scripts/download_xiaohongshu.py "https://www.xiaohongshu.com/explore/图文帖ID" -o content/

# 批量下载
python3 scripts/download_xiaohongshu.py --batch download_list.md -o content/

# 下载 720p 画质
python3 scripts/download_xiaohongshu.py "链接" -q 720p -o content/

# 仅下载音频
python3 scripts/download_xiaohongshu.py "链接" -a -o content/

# 使用 Firefox Cookie
python3 scripts/download_xiaohongshu.py "链接" --browser firefox -o content/
```
...
6. 通过三级策略获取字幕
7. 从字幕生成纯文本转录
8. 保存 `.meta.json` 元数据供 Claude 生成 `summary.md`

对于图文帖，工具会：

5. 使用移动端 User-Agent 解析帖子页面，提取所有图片链接
6. 下载每张图片，并将正文保存为 `post.txt`
7. 保存 `.meta.json` 供 Claude 读取全部图片并生成 `summary.md`
...
├── README.md                             # 本文件
├── LICENSE                               # MIT 许可证
├── download_list.md                      # 批量下载队列
├── .gitignore
├── scripts/
│   ├── download_xiaohongshu.py           # 主下载脚本（视频 + 图文）
│   └── parallel_transcribe.py            # 并行 Whisper 转录
└── reference/
    ├── summary-prompt.md                 # 视频帖 AI 总结提示词模板
    └── image-summary-prompt.md           # 图文帖 AI 总结提示词模板
```
